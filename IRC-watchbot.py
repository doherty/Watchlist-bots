			...
			if len(a) == 2:
				...
				if self.getCloak(e.source()) in config.get('Setup', 'privileged').split('<|>'):
					try:
						...
						self.msg('You have to follow the proper syntax. See \x0302[[tswiki:User:Mike.lifeguard/IRC-watchbot]]\x03.', nick)
	...
	def do_command(self, e, cmd, target):
		...
		if args[0] == 'test':#Notifications
			...
			if args[1] == 'list':
				...
				self.msg('You have to specify a channel. Use irc.wikimedia.org\'s channel name, "all" or "metaonly".', target)
