...
##        print timestamp+" * "+who+a
			...
			if len(a) == 2:
				...
				if self.getCloak(e.source()) in config.get('Setup', 'privileged').split('<|>'):
					try:
						...
						self.msg('You have to follow the proper syntax. See \x0302[[tswiki:User:Mike.lifeguard/The_Hawk]]\x03.', nick)
	...
	def do_command(self, e, cmd, target):
		...
		if args[0] == 'test':#Notifications
			...
			if args[1] == 'list':
				...
				for section in config.sections():
					if section != 'Setup' and section != '#en.wikibooks':
						self.setConfig(section, 'report', 'False')
				self.msg('Setting report level for all configured channels except #en.wikibooks to False.', target)
	...
	def saveConfig(self):
		...
		configFile = open(os.path.expanduser('~/Watchlist-bots/The_Hawk.ini'), 'w')
